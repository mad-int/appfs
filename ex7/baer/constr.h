 ...
 * Check if Ax <= b (x is a feasible solution to the constraints).
 *
 * But x should be different from the previous x by one bitflip.
 * state should point to the pointer of the internal state.
 * On the first call *state should be NULL.
 *
 * *state needs to be free()d at the end!
 */
...
void reorder_rows_failfirst(constraints_t* cs);
