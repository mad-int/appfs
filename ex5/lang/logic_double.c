...
problem* process_file_double (const char* filename)
{   
    ...
    while(NULL != (s = fgets(buf, sizeof(buf), fp))) {
        ...
        check_line_double(s, MAX_LINE_LEN, line_num);
...
void print_solutions_double(problem* instance) {
    
    printf("matrix:\n");
    for (int i = 0; i < instance->rows; i++) {
        printf("\n");
        for (int j = 0; j < instance->columns; j++) {
            printf("%f ", instance->matrix[i][j]);
        }
    }
    printf("\n\nright hand side:\n\n");
    for (int i = 0; i < instance->rows; i++) {
    ...
    for (int i = 0; i < pow(2, instance->columns); i++) {
        ...
        for (int j = 0; j < instance->rows; j++) {
            double product = (scalar_product(x, instance->matrix[j], instance->columns));
